...
export async function POST(request: Request) {
  ...
  if (new Date(transfer.expiresAt) < new Date()) {
    await supabase.from("ownership_transfers").update({ status: "EXPIRED" }).eq("id", transfer.id);
  ...
  // Ensure the accepting user has (or gets) a membership, then move them into
  // the OWNER role. If they weren't already a member, create the membership now.
