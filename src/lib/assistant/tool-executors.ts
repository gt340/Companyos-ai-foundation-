...
// The model often has no way to see a campaign's real UUID in the
// conversation (e.g. right after creating one, only its name/channel/
// status were shown back) — so any tool taking a campaignId accepts
// either the real ID or the campaign's name, resolved here. Same pattern
// as resolveStageId for Sales pipeline stages.
...
async function resolveCampaignId(
  ...
  if (UUID_PATTERN.test(campaignIdOrName)) return campaignIdOrName;

  const campaign = await prisma.campaign.findFirst({
    where: { organizationId, name: { equals: campaignIdOrName, mode: "insensitive" } },
  });
  if (!campaign) {
    throw new Error(`No campaign found matching "${campaignIdOrName}" (checked by ID and by name).`);
  }
  return campaign.id;
}
...
async function getCampaign(args: { campaignId: string }, ctx: ExecutorContext) {
  const campaignId = await resolveCampaignId(ctx.organizationId, args.campaignId);
  const campaign = await prisma.campaign.findFirst({
...
async function listContentItems(
  ...
  const campaignId = args.campaignId
    ? await resolveCampaignId(ctx.organizationId, args.campaignId)
    : undefined;
...
async function createContentItem(
  ...
  return withAgentApproval(ctx, ctx.marketingAgentId, "create_content_item", args, async () => {
    const campaignId = args.campaignId
      ? await resolveCampaignId(ctx.organizationId, args.campaignId)
      : undefined;
...
// ── MARKETING AGENT: AI capability tools ────────────────────────────────
// Same shape as runSalesAiPrompt — load marketing_system + one
// purpose-specific PromptTemplate, hand the model only real data pulled
// from Company/MarketingProfile/Campaign/ContentItem/Competitor, return
// plain text. Grounding discipline lives in the template content itself.
async function runMarketingAiPrompt(
  templatePurpose: string,
  contextText: string,
  extraInstruction?: string
): Promise<string> {
  const [systemTemplate, promptTemplate] = await Promise.all([
    prisma.promptTemplate.findFirst({
      where: { purpose: "marketing_system", agentType: "MARKETING", isActive: true },
    }),
    prisma.promptTemplate.findFirst({
      where: { purpose: templatePurpose, agentType: "MARKETING", isActive: true },
    }),
  ]);
  ...
  const completion = await openai.chat.completions.create({
    model: "gpt-4o",
    messages: [
      { role: "system", content: systemTemplate?.content ?? "You are the Marketing Agent." },
      {
        role: "user",
        content: `${promptTemplate.content}\n\n${
          extraInstruction ? `ADDITIONAL INSTRUCTION FROM THE USER: ${extraInstruction}\n\n` : ""
...
// Shared brand/company context every Marketing AI tool draws from —
// Company's existing marketing fields + MarketingProfile, never duplicated.
async function buildBrandContext(organizationId: string): Promise<string> {
  const [company, organization, profile] = await Promise.all([
    prisma.company.findUnique({ where: { organizationId } }),
    prisma.organization.findUnique({ where: { id: organizationId } }),
    prisma.marketingProfile.findUnique({ where: { organizationId } }),
  ]);
  ...
  return [
    ...
    `Brand voice: ${company?.brandVoice ?? "Not set"}`,
    ...
    `Known competitors (names only): ${company?.competitors?.length ? company.competitors.join("; ") : "Not set"}`,
    `Brand guidelines: ${profile?.brandGuidelines ?? "Not set"}`,
    `Positioning statement: ${profile?.positioningStatement ?? "Not set"}`,
    `Default tone: ${profile?.defaultTone ?? "Not set"}`,
  ].join("\n");
}

async function generateMarketingStrategy(
  args: { focusArea?: string },
  ctx: ExecutorContext
) {
  const brandContext = await buildBrandContext(ctx.organizationId);
  const analysis = await runMarketingAiPrompt("marketing_strategy", brandContext, args.focusArea);
  await logAgentAction(ctx, ctx.marketingAgentId, "generate_marketing_strategy", args, { analysis }, null, "strategy generated");
  return { analysis };
}

async function generateCampaignPlan(
  args: { objective?: string; channel?: string },
  ctx: ExecutorContext
) {
  const brandContext = await buildBrandContext(ctx.organizationId);
  const instruction = [
    ...
    .filter(Boolean)
    .join(" | ");
  const analysis = await runMarketingAiPrompt("campaign_planning", brandContext, instruction || undefined);
  await logAgentAction(ctx, ctx.marketingAgentId, "generate_campaign_plan", args, { analysis }, null, "plan generated");
  return { analysis };
}
...
async function generateContentStrategy(
  ...
  const [brandContext, recentContent] = await Promise.all([
    buildBrandContext(ctx.organizationId),
    prisma.contentItem.findMany({
      where: { organizationId: ctx.organizationId },
      orderBy: { createdAt: "desc" },
      take: 10,
      select: { type: true, title: true, status: true, performanceNotes: true },
    }),
  ]);
  ...
  const contextText = [
    ...
    recentContent.length
      ? `Recent content history:\n${recentContent
          .map((c) => `- [${c.type}] "${c.title}" (${c.status})${c.performanceNotes ? ` — ${c.performanceNotes}` : ""}`)
          .join("\n")}`
      : "No content history recorded yet.",
  ].join("\n\n");

  const analysis = await runMarketingAiPrompt("content_strategy", contextText, args.focusArea);
  await logAgentAction(ctx, ctx.marketingAgentId, "generate_content_strategy", args, { analysis }, null, "strategy generated");
  return { analysis };
}

async function draftContent(
  args: { type: string; brief: string; platform?: string; campaignId?: string },
  ctx: ExecutorContext
) {
  const [brandContext, campaign] = await Promise.all([
    buildBrandContext(ctx.organizationId),
    args.campaignId
      ? resolveCampaignId(ctx.organizationId, args.campaignId).then((id) =>
  ...
  const contextText = [
    brandContext,
    `Requested content type: ${args.type}`,
    args.platform ? `Target platform: ${args.platform}` : "Target platform: not specified",
    campaign
      ? `Related campaign: "${campaign.name}" (objective: ${campaign.objective ?? "not set"}, channel: ${campaign.channel})`
      : "Not tied to a specific campaign.",
  ].join("\n");

  const draft = await runMarketingAiPrompt("content_generation", contextText, args.brief);
  await logAgentAction(ctx, ctx.marketingAgentId, "draft_content", args, { draft }, null, "draft generated");
  return { type: args.type, platform: args.platform, campaignId: campaign?.id, draft };
}

async function analyzeCompetitor(args: { competitorId: string }, ctx: ExecutorContext) {
  const competitor = await prisma.competitor.findFirst({
    where: { id: args.competitorId, organizationId: ctx.organizationId },
  });
  if (!competitor) throw new Error("Competitor not found in this organization.");

  const brandContext = await buildBrandContext(ctx.organizationId);
  const contextText = [
    brandContext,
    `Competitor: ${competitor.name}`,
    `Website: ${competitor.website ?? "Not recorded"}`,
    `Notes: ${competitor.notes ?? "Not recorded"}`,
    `Strengths: ${competitor.strengths ?? "Not recorded"}`,
    `Weaknesses: ${competitor.weaknesses ?? "Not recorded"}`,
    `Last researched: ${competitor.lastResearchedAt?.toISOString() ?? "Never"}`,
  ].join("\n");

  const analysis = await runMarketingAiPrompt("competitor_analysis", contextText);
  await logAgentAction(ctx, ctx.marketingAgentId, "analyze_competitor", args, { analysis }, null, "analysis generated");
  return { competitorId: competitor.id, competitorName: competitor.name, analysis };
}

async function analyzeMarketingPerformance(args: { campaignId: string }, ctx: ExecutorContext) {
  const campaignId = await resolveCampaignId(ctx.organizationId, args.campaignId);
  const campaign = await prisma.campaign.findFirst({
    where: { id: campaignId, organizationId: ctx.organizationId },
  });
  ...
  const cost = campaign.costToDate !== null ? Number(campaign.costToDate) : null;
  const revenue = campaign.revenue !== null ? Number(campaign.revenue) : null;
  const roi = cost !== null && revenue !== null && cost > 0 ? Math.round(((revenue - cost) / cost) * 100) : null;

  const contextText = [
    `Campaign: ${campaign.name} (${campaign.channel}, status ${campaign.status})`,
    `Budget: ${campaign.budget ?? "Not recorded"}`,
    `Cost to date: ${cost ?? "Not recorded"}`,
    `Impressions: ${campaign.impressions}`,
    `Reach: ${campaign.reach}`,
    `Engagement (count): ${campaign.engagement}`,
    `Clicks: ${campaign.clicks}`,
    `Conversions: ${campaign.conversions}`,
    `Revenue: ${revenue ?? "Not recorded"}`,
    `ROI: ${roi !== null ? `${roi}%` : "Not available — cost and/or revenue not recorded"}`,
  ].join("\n");

  const analysis = await runMarketingAiPrompt("marketing_performance_analysis", contextText);
  await logAgentAction(ctx, ctx.marketingAgentId, "analyze_marketing_performance", args, { analysis }, null, "analysis generated");
  return { campaignId: campaign.id, campaignName: campaign.name, roiPercent: roi, analysis };
}

// Reuses the existing ExecutiveReport/ReportSection tables (already
// agent-agnostic) rather than a new Marketing-specific report table — no
// schema change, no duplicated infrastructure. Same JSON-shape
// enforcement and atomic save/fail pattern as the CEO's report engine,
// but gathers marketing context instead of KB/CEO-memory context.
async function generateMarketingReport(
  args: { title?: string; reportingPeriod?: string; focusArea?: string },
  ctx: ExecutorContext
) {
  const [brandContext, campaigns, competitors, systemTemplate, reportTemplate] = await Promise.all([
    buildBrandContext(ctx.organizationId),
    prisma.campaign.findMany({ where: { organizationId: ctx.organizationId }, take: 20, orderBy: { createdAt: "desc" } }),
    prisma.competitor.findMany({ where: { organizationId: ctx.organizationId }, take: 10 }),
    prisma.promptTemplate.findFirst({ where: { purpose: "marketing_system", agentType: "MARKETING", isActive: true } }),
    prisma.promptTemplate.findFirst({ where: { purpose: "marketing_report", agentType: "MARKETING", isActive: true } }),
  ]);

  if (!reportTemplate) throw new Error('The "marketing_report" prompt template isn\'t set up yet.');

  const campaignContext = campaigns.length
    ? campaigns
        .map((c) => {
          const cost = c.costToDate !== null ? Number(c.costToDate) : null;
          const revenue = c.revenue !== null ? Number(c.revenue) : null;
          return `- ${c.name} (${c.channel}, ${c.status}): impressions ${c.impressions}, clicks ${c.clicks}, conversions ${c.conversions}, cost ${cost ?? "not recorded"}, revenue ${revenue ?? "not recorded"}`;
        })
        .join("\n")
    : "No campaigns recorded yet.";

  const competitorContext = competitors.length
    ? competitors.map((c) => `- ${c.name}: ${c.notes ?? "no notes recorded"}`).join("\n")
    : "No competitors recorded yet.";

  const report = await prisma.executiveReport.create({
    data: {
      organizationId: ctx.organizationId,
      agentId: ctx.marketingAgentId,
      generatedBy: ctx.userId,
      title: args.title || `Marketing Report — ${new Date().toLocaleDateString()}`,
...
Return ONLY a JSON object shaped exactly like this:
{
  ...
  "sections": [
    { "title": "string", "sectionType": "SUMMARY|CAMPAIGNS|CONTENT|COMPETITORS|PERFORMANCE|RECOMMENDATION|DATA_GAP", "content": "string, include FACT/ANALYSIS/INSIGHT/RECOMMENDATION/DATA GAP labels inline where relevant", "priority": 1-5, "sourceRefs": ["string", ...] }
  ]
}
Omit sections with genuinely nothing to say rather than padding with empty content.`,
    ...
    let parsed: { summary?: string; sections?: { title: string; sectionType: string; content: string; priority?: number; sourceRefs?: string[] }[] };
    try {
    ...
    return { reportId: report.id, title: report.title, summary: parsed.summary, sectionTitles: sections.map((s) => s.title) };
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    await prisma.executiveReport.update({ where: { id: report.id }, data: { status: "FAILED", errorMessage: message } });
    throw err;
  }
}
...
export const TOOL_EXECUTORS: Record<string, ExecutorFn> = {
  ...
  // Marketing Agent (Phase 8)
  get_marketing_profile: getMarketingProfile,
  list_campaigns: listCampaigns,
  get_campaign: getCampaign,
  list_content_items: listContentItems,
  get_content_item: getContentItem,
  list_competitors: listCompetitors,
  update_marketing_profile: updateMarketingProfile,
  create_campaign: createCampaign,
  update_campaign: updateCampaign,
  update_campaign_metrics: updateCampaignMetrics,
  ...
  generate_marketing_strategy: generateMarketingStrategy,
  generate_campaign_plan: generateCampaignPlan,
  generate_content_strategy: generateContentStrategy,
